...
use crate::async_fuse::memfs::persist::PersistError;
...
pub struct S3Node<S: S3BackEnd + Sync + Send + 'static> {
    ...
    /// Full path
    full_path: String,
    /// S3Node attribute, shared between parent directory and child node
...
impl<S: S3BackEnd + Send + Sync + 'static> S3Node<S> {
    ...
    pub fn new_child_node_of_parent(
        ...
        let child_kind = child_attr.read().kind;
        let data = match child_kind {
            ...
            _ => panic!("unsupported type {child_kind:?}"),
        };
        let full_path = parent.absolute_path_of_child(child_name, child_kind);
        Self {
    ...
    pub(crate) fn _set_attr(&mut self, new_attr: FileAttr, _broadcast: bool) -> FileAttr {
        ...
        */
        let mut write = self.attr.write();
        write.clone_from(&new_attr);
        old_attr
    }
            ...
            Ok(mut root) => {
                if let Some(root_attr) = root.persist_serialized.root_attr.take() {
                    Ok(Self::new(
                        ...
                        name,
                        "/".to_owned(),
                        Arc::new(RwLock::new(serial::serial_to_file_attr(&root_attr))),
                        root.new_s3_node_data_dir(),
                        s3_backend,
                        meta,
                    ))
                } else {
                    ...
                    Err(anyhow::Error::from(PersistError::FileAttrMissingForRoot))
                }
            }
            Err(e) => {
...
impl<S: S3BackEnd + Sync + Send + 'static> Node for S3Node<S> {
    ...
    async fn create_child_symlink(
        ...
        let target_path = {
            ...
            let entry = DirEntry::new(child_symlink_name.to_owned(), Arc::clone(&child_attr));
    ...
    async fn load_child_symlink(
        ...
        // get target path of link by reading s3 backend
