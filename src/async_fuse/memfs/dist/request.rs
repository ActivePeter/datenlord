...
pub fn read(file_name: Vec<u8>, index: Vec<Index>) -> Vec<u8> {
    bincode::serialize(&DistRequest::Read(OpArgs { file_name, index }))
...
pub fn load_dir(path: &str) -> Vec<u8> {
    bincode::serialize(&DistRequest::LoadDir(path.to_owned()))
        .unwrap_or_else(|e| panic!("fail to serialize `LoadDir` distributed meta operation, {e}"))
}
...
pub fn update_dir(
    ...
    bincode::serialize(&DistRequest::UpdateDir(args))
        .unwrap_or_else(|e| panic!("fail to serialize `UpdateDir` distributed meta operation, {e}"))
}
...
pub fn remove_dir_entry(parent: &str, child: &str) -> Vec<u8> {
    ...
    bincode::serialize(&DistRequest::RemoveDirEntry(args)).unwrap_or_else(|e| {
        panic!("fail to serialize `RemoveDirEntry` distributed meta operation, {e}")
    })
}
...
pub fn get_file_attr(path: &str) -> Vec<u8> {
    bincode::serialize(&DistRequest::GetFileAttr(path.to_owned())).unwrap_or_else(|e| {
        panic!("fail to serialize `GetFileAttr` distributed meta operation, {e}")
    })
}
...
pub fn push_file_attr(path: &str, attr: SerialFileAttr) -> Vec<u8> {
    bincode::serialize(&DistRequest::PushFileAttr((path.to_owned(), attr))).unwrap_or_else(|e| {
        panic!("fail to serialize `PushFileAttr` distributed meta operation, {e}")
    })
}
