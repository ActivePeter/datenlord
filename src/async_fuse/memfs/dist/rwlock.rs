...
impl<M: MetaData + Send + Sync + 'static> DistRwLockManager<M> {
    ...
    pub fn new(
        etcd: Arc<EtcdDelegate>,
        metadata: Weak<M>,
    ) -> (JoinHandle<()>, Arc<DistRwLockManager<M>>) {
    ...
    fn metadata(&self) -> Arc<M> {
        self.metadata.upgrade().unwrap_or_else(|| {
            panic!("metadata should lives longer than it's sub module");
        })
    }
    fn set_timeout(
        &self,
    ...
    /// Lock dist rwlock
    pub async fn lock(&self, name: &str, locktype: DistRwLockType) -> DatenLordResult<()> {
    ...
    /// Unlock dist rwlock
    pub async fn unlock(&self, name: &str) -> DatenLordResult<()> {
        dist_rwlock::rw_unlock(&self.etcd, name, self.metadata().node_id()).await
    }
    /// Stop the async task
